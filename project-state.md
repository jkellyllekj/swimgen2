...
### UI – Settings wheel and Share (2026-03-11)
- **Top row (generator):** Unchanged. Colour picker (teardrop), background (picture frame), and help (?) stay in place.
- **Workout title row:** Lock and dolphin stay. Colour picker and background buttons were removed from this row and replaced with **Settings** (gear) and **Share**. Order: **[lock] [dolphin] [settings] [share] [workout name]**.
- **Settings panel:** Opened from the workout row settings button. Contains: **Colour picker** (same `setBgColor` behaviour), **Background** (cycle image + solid colour, same as before), **Help** (“How to use SwimSum” / replay onboarding). Close via overlay click or Close button.
- **Share:** Tapping Share opens a menu: **Share to socials** (screenshot of workout + link via native share sheet – Facebook, WhatsApp, Instagram, TikTok, etc.) and **Print** (same workout region with “Generated with SwimSum – https://swimsum.com” at bottom). Screenshot/print scope: from the workout title bar down through all workout content (full scrollable area). Implemented with `#workoutCaptureRegion`, html2canvas (CDN), and `@media print` CSS. Revert: branch `feature/settings-wheel-and-share`; to roll back use `cursor-transition` or revert the merge. See `CHANGELOG-SETTINGS-SHARE.md` for file-level change list.
