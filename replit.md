...
- **S24 Pop-Back Fix**: Touch-primary drag architecture. setPointerCapture only for mouse (not touch) to avoid browser pointercancel conflict with touch-action:pan-y. touchmove/touchend drive drag on mobile; pointermove/pointerup on desktop. dragCommitted flag prevents double-fire. pointercancel is no-op for touch (touchend handles commit).
- **Freeze Prevention**: CSS touch-action on [data-effort] is 'pan-y' (allows native scroll). Only set to 'none' inline on the card during active long-press drag. No setPointerCapture for touch avoids the spec conflict where browser cancels pointer capture to enable panning.
